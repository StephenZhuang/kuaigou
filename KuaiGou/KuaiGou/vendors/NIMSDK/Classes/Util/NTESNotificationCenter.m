    ...
    if (data)
    {
        ...
        if ([dict isKindOfClass:[NSDictionary class]])
        {
            if ([dict jsonInteger:NTESNotifyID] == NTESCustom)
            {
                ...
                [[UIApplication sharedApplication].keyWindow makeToast:content duration:2.0 position:CSToastPositionCenter];
            }
        }
    }
}
...
- (void)onReceive:(UInt64)callID from:(NSString *)caller type:(NIMNetCallType)type{
    ...
    KGTabBarController *tabVC = (KGTabBarController *)[UIApplication sharedApplication].keyWindow.rootViewController;
             ...
             message:(NSString *)info
{
    ...
    if (tabVC.presentedViewController) {
        __weak KGTabBarController *wtabVC = (KGTabBarController *)tabVC;
