...
//  NIM
//
//  Created by chrisRay on 15/5/18.
...
@property (nonatomic,strong) M80TimerHolder *timer;
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil{
    ...
    if (self) {
        ...
        if (!self.callInfo) {
            [AudioUtil setDefaultSpeakerMode:YES];
            _callInfo = [[NetCallChatInfo alloc] init];
        }
        _timer = [[M80TimerHolder alloc] init];
    }
    return self;
}
...
- (void)setUpStatusBar{
    if(!IOS7) {
        UIStatusBarStyle style = UIStatusBarStyleBlackTranslucent;
